 ...
 *   BING_SITE       optional: restrict to one site url
...
async function main() {
  ...
  let sites = (userSites || []).map((s) => s.Url).filter(Boolean);
  if (!sites.length) {
  ...
  if (process.env.BING_SITE) {
    if (!sites.includes(process.env.BING_SITE)) {
      throw new Error(`BING_SITE="${process.env.BING_SITE}" is not among the verified sites: ${sites.join(', ')}`);
    }
    sites = [process.env.BING_SITE];
  }
